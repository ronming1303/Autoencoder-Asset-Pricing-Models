...
class modelBase:
    def __init__(self, name):
        ...
        self.train_idx = 0
        self.refit_cnt = 0  # refit count, used to record how many times the model has been refitted
    ...
    def predict(self, month):
        ...
        lag_factor, mon_beta = self.cal_delayed_Factor(
            month), self.calBeta(month)

        assert mon_beta.shape[1] == lag_factor.shape[
            0], f"Dimension mismatch between lag_factor: {lag_factor.shape} and mon_beta: {mon_beta.shape}"
    ...
    def refit(self):
        ...
        # rolling training to the next year
        self.train_period = (pd.Series(self.train_period) + 10000).to_list()
