...
class CA_base(nn.Module, modelBase):
    def __init__(self, name, omit_char=[], device='mps'):
    ...
    def _get_item(self, month):
        if month not in self.p_charas['DATE'].values:
            ...
            month = self.p_charas['DATE'].values[np.argmin(
                np.abs(self.p_charas['DATE'].values - month))]
    ...
    def dataloader(self, period):
        ...
        mon_list = mon_list.loc[(mon_list >= period[0])
                                & (mon_list <= period[1])]
        beta_nn_input_set = []
        factor_nn_input_set = []
        label_set = []
    ...
    def calBeta(self, month, skip_char=[]):
        _, beta_nn_input, _, _ = self._get_item(
            month)  # beta input: 94*94 = P*N

        # if some variables need be omitted
        if len(skip_char):
            beta_nn_input = pd.DataFrame(
                beta_nn_input.T, columns=CHARAS_LIST)  # N*P
    ...
    def calFactor(self, month, skip_char=[]):
        ...
        # if some variables need be omitted
        if len(skip_char):
            factor_nn_input = pd.DataFrame(
                factor_nn_input.T, columns=CHARAS_LIST)  # 1*P
    ...
    def inference(self, month):
        if len(self.omit_char) == 0:
            assert month >= self.test_period[
                0], f"Month error, {month} is not in test period {self.test_period}"
            ...
            for char in self.omit_char:
                mon_factor, mon_beta = self.calFactor(
                    month, [char]), self.calBeta(month, [char])
                ret_R.append(
                    (mon_beta @ mon_factor).cpu().detach().numpy())  # N*1

            mon_factor, mon_beta = self.calFactor(month), self.calBeta(month)
            # also add complete result
    ...
    def cal_delayed_Factor(self, month):
        ...
        if self.refit_cnt == 0:
            # input of the first predict take hat{f}_t
            avg_f_pred = self.factor_nn_pred[0]
            # print(avg_f_pred.shape)
        else:
            avg_f_pred = torch.mean(torch.stack(
                self.factor_nn_pred[:self.refit_cnt]), dim=0)
    ...
    def reset_weight(self):
        ...
        self.optimizer.state = collections.defaultdict(
            dict)  # reset optimizer state
...
class CA0(CA_base):
    def __init__(self, hidden_size, lr=0.001, omit_char=[], device='mps'):
        CA_base.__init__(
            self, name=f'CA0_{hidden_size}', omit_char=omit_char, device=device)
        # P -> K
...
class CA1(CA_base):
    def __init__(self, hidden_size, dropout=0.5, lr=0.001, omit_char=[], device='mps'):
        CA_base.__init__(
            self, name=f'CA1_{hidden_size}', omit_char=omit_char, device=device)
...
class CA2(CA_base):
    def __init__(self, hidden_size, dropout=0.5, lr=0.001, omit_char=[], device='mps'):
        CA_base.__init__(
            self, name=f'CA2_{hidden_size}', omit_char=omit_char, device=device)
        self.dropout = dropout
        # P -> 32 -> 16 -> K
        self.beta_nn = nn.Sequential(
            # hidden layer 1
...
class CA3(CA_base):
    def __init__(self, hidden_size, dropout=0.5, lr=0.001, omit_char=[], device='mps'):
        CA_base.__init__(
            self, name=f'CA3_{hidden_size}', omit_char=omit_char, device=device)
        self.dropout = dropout
        # P -> 32 -> 16 -> 8 -> K
        self.beta_nn = nn.Sequential(
            # hidden layer 1
