...
class IPCA(modelBase):
    def __init__(self, K, omit_char=[]):
        ...
        np.random.seed(10)
        # P = 94, we have total 94 characteristics
        self.gamma = np.random.random([94, self.K])
        self.valid_error = []
        self.__prepare_data()
    ...
    def __prepare_data(self):
        ...
        self.p_charas = pd.read_pickle(
            'data/p_charas.pkl')  # not clear what is this
    ...
    def __valid(self):
        ...
        for mon in self.mon_list[(self.mon_list >= self.valid_period[0]) & (self.mon_list <= self.valid_period[1])]:
            Z = self.p_charas.loc[self.p_charas.DATE ==
                                  mon][CHARAS_LIST].values  # N * P
            # N * 1
            y = self.portfolio_ret.loc[self.portfolio_ret.DATE ==
                                       mon][CHARAS_LIST].values.T
    ...
    def inference(self, month):
        if not len(self.omit_char):
            Z = self.p_charas.loc[self.p_charas.DATE ==
                                  month][CHARAS_LIST].values  # N * P
            y = self.portfolio_ret.loc[self.portfolio_ret.DATE ==
                                       month][CHARAS_LIST].values.T  # N * 1
            beta = Z @ self.gamma  # N * K
            f_hat = np.array(np.matrix(beta.T @ beta).I @ beta.T @ y)  # K * 1
            ...
            inference_R = []
            Z = self.p_charas.loc[self.p_charas.DATE ==
                                  month][CHARAS_LIST].copy(deep=False)
            y = self.portfolio_ret.loc[self.portfolio_ret.DATE == month][CHARAS_LIST].copy(
                deep=False)
            ...
            for char in self.omit_char:
                ...
                f_hat = np.array(np.matrix(beta.T @ beta).I @
                                 beta.T @ y_input)  # K * 1
    ...
    def predict(self, month):
        # 如果 refit_cnt 为 0，直接调用 inference。
        # 否则，使用历史因子收益的平均值进行预测。
        if self.refit_cnt == 0:
