...
class PCA(modelBase):
    ...
    def train_model(self):
        pr = self.portfolio_ret.loc[(self.portfolio_ret.DATE >= self.train_period[0]) & (
            self.portfolio_ret.DATE <= self.train_period[1])][CHARAS_LIST].values
        # col demean [均值化处理参考](https://www.zhihu.com/question/40956812)
        pr = pr - np.mean(pr, axis=0)
        ret_cov_matrix = np.zeros((pr.shape[1], pr.shape[1]))

        for i in range(pr.shape[0]):  # Sum of y^t @ y^t.T
            ret_cov_matrix += (pr[i, :].reshape(-1, 1) @
                               pr[i, :].reshape(-1, 1).T)
        ret_cov_matrix = ret_cov_matrix/(pr.shape[0]*pr.shape[1])  # N * T

        eigVal, eigVec = np.linalg.eig(ret_cov_matrix)
        sorted_indices = np.argsort(eigVal)
        # Beta: N * K 这段代码的作用是 从特征向量矩阵 eigVec 中选择对应最大 K 个特征值的特征向量，并赋值给 self.beta
        self.beta = eigVec[:, sorted_indices[:-self.K-1:-1]]

    def calBeta(self, month):
        return np.real(self.beta)

    def calFactor(self, month):
        tr = self.portfolio_ret.loc[self.portfolio_ret.DATE <=
                                    month].iloc[-1][CHARAS_LIST].values
        tr = tr - np.mean(tr, axis=0)  # col demean
        # print(tr)
        factor = np.array(
            (np.matrix(self.beta.T @ self.beta).I @ self.beta.T) @ tr.T).T  # K * 1
    ...
    def cal_delayed_Factor(self, month):
        ...
        tr = self.portfolio_ret.loc[(self.portfolio_ret.DATE >= 19870101) & (
            self.portfolio_ret.DATE < month)][CHARAS_LIST].values
        tr = tr - np.mean(tr, axis=0)  # col demean
        ...
        for i in range(tr.shape[0]):
            factors.append(
                np.array(np.matrix(self.beta.T @ self.beta).I @ self.beta.T @ tr[i, :]))
