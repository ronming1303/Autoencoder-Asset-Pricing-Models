...
def model_inference_and_predict(model):
    ...
    inference_result = []
    predict_result = []
    T_bar = tqdm(test_mons.groupby(test_mons.apply(lambda x: x//10000)),  # 每一年做一次inference and predicting
                 colour='red', desc=f'{model.name} Inferencing & Predicting')
    ...
    for g in T_bar:  # rolling train
        ...
        # model refit (change train period and valid period) # train, valid, predict数据集整体向后滑动一年
        model.refit()

    if not len(model.omit_char):
        inference_result = pd.DataFrame(
            inference_result, index=test_mons, columns=CHARAS_LIST)
        inference_result.to_csv(
            f'results/inference/{model.name}_inference.csv')
        predict_result = pd.DataFrame(
            predict_result, index=test_mons, columns=CHARAS_LIST)
...
def model_inference_and_predict_CA(model):
    ...
    T_bar = tqdm(test_mons.groupby(test_mons.apply(lambda x: x//10000)),
                 colour='red', desc=f'{model.name} Inferencing & Predicting')
    ...
    for g in T_bar:  # rolling train, refit once a year
        ...
        for m in g[1].to_list():
            ...
            stock_index = pd.concat([stock_index, pd.Series(
                m_stock_index)]).drop_duplicates().astype(int)

            if not len(model.omit_char):  # no omit characteristics
                # move inference_R and predict_R to cpu
                inference_R = model.inference(m)  # return (N, 1)
                inference_R = inference_R.cpu().detach().numpy()
                inference_R = pd.DataFrame(
                    inference_R, index=m_stock_index, columns=[m])
                inference_result = pd.concat([inference_result.reset_index(
                    drop=True), inference_R.reset_index(drop=True)], axis=1)  # (N, T)

                predict_R = model.predict(m)  # reutrn (N, 1)
                predict_R = predict_R.cpu().detach().numpy()
                predict_R = pd.DataFrame(
                    predict_R, index=m_stock_index, columns=[m])
                predict_result = pd.concat([predict_result.reset_index(
                    drop=True), predict_R.reset_index(drop=True)], axis=1)  # (N, T)

            else:
                # return (N, m), m is the length of omit_char
    ...
    if not len(model.omit_char):
        inference_result = pd.DataFrame(
            inference_result.values.T, index=test_mons, columns=CHARAS_LIST)
        inference_result.to_csv(
            f'results/inference/{model.name}_inference.csv')

        predict_result = pd.DataFrame(
            predict_result.values.T, index=test_mons, columns=CHARAS_LIST)
        predict_result.to_csv(f'results/predict/{model.name}_predict.csv')
...
def model_selection(model_type, model_K, omit_char=[]):
    assert model_type in ['FF', 'PCA', 'IPCA', 'CA0',
                          'CA1', 'CA2', 'CA3'], f'No Such Model: {model_type}'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--Model', type=str,
                        default='FF PCA IPCA CA0 CA1 CA2 CA3')
    ...
    for g in product(args.Model.split(' '), args.K.split(' ')):
        ...
        print(
            f"{time.strftime('%a, %d %b %Y %H:%M:%S +0800', time.gmtime())} | Model: {model['name']} | {omit_chars}")
        ...
        if not len(model['omit_char']):
            ...
            for i in range(len(model['omit_char'])):
                ...
                R_square.append(calculate_R2(
                    None, None, inference_r, complete_r))
